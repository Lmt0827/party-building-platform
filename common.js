...
(function() {
  ...
  // 新版标签选择器初始化（与企业新增标签配置保持一致）
  function initTagSelectors(root) {
    root = root || document;
    root.querySelectorAll('.tag-selector').forEach(function(selector) {
      if (selector._tagSelectorBound) return;
      selector._tagSelectorBound = true;

      var tagPool = [];
      selector.querySelectorAll('.available-tags-list .industry-tag').forEach(function(tag) {
        tagPool.push(tag.getAttribute('data-value') || tag.textContent.replace('+', '').trim());
      });
      selector.querySelectorAll('.selected-tags .industry-tag').forEach(function(tag) {
        var val = tag.getAttribute('data-value') || tag.textContent.replace('×', '').trim();
        if (tagPool.indexOf(val) < 0) tagPool.push(val);
      });

      function render() {
        var selectedTags = [];
        selector.querySelectorAll('.selected-tags .industry-tag').forEach(function(tag) {
          selectedTags.push(tag.getAttribute('data-value') || tag.textContent.replace('×', '').trim());
        });

        var selectedContainer = selector.querySelector('.selected-tags');
        if (selectedTags.length === 0) {
          selectedContainer.innerHTML = '<span class="tag-placeholder">暂无标签，点击下方可选标签添加</span>';
        } else {
          selectedContainer.innerHTML = selectedTags.map(function(tag) {
            return '<span class="industry-tag removable" data-value="' + tag + '">' + tag + '<span class="tag-close">&times;</span></span>';
          }).join('');
        }

        var availableContainer = selector.querySelector('.available-tags-list');
        var availableOptions = tagPool.filter(function(opt) { return selectedTags.indexOf(opt) < 0; });
        if (availableOptions.length === 0) {
          availableContainer.innerHTML = '<span class="tag-placeholder">所有标签已选中</span>';
        } else {
          availableContainer.innerHTML = availableOptions.map(function(opt) {
            return '<span class="industry-tag available" data-value="' + opt + '">+ ' + opt + '</span>';
          }).join('');
        }
      }

      selector.addEventListener('click', function(e) {
        var availableTag = e.target.closest('.available-tags-list .industry-tag.available');
        if (availableTag) {
          var val = availableTag.getAttribute('data-value') || availableTag.textContent.replace('+', '').trim();
          var selectedContainer = selector.querySelector('.selected-tags');
          if (selectedContainer.querySelector('.tag-placeholder')) {
            selectedContainer.innerHTML = '';
          }
          var span = document.createElement('span');
          span.className = 'industry-tag removable';
          span.setAttribute('data-value', val);
          span.innerHTML = val + '<span class="tag-close">&times;</span>';
          selectedContainer.appendChild(span);
          render();
          return;
        }

        var removeTag = e.target.closest('.selected-tags .industry-tag.removable');
        if (removeTag) {
      ...
      var newInput = selector.querySelector('.tag-new-input');
      if (newInput) {
        newInput.addEventListener('keydown', function(e) {
          if (e.key === 'Enter') {
            e.preventDefault();
            var tagName = newInput.value.trim();
            if (!tagName) {
              showToast('请输入标签名称', 'error');
              return;
            }
            if (tagPool.indexOf(tagName) < 0) {
              tagPool.push(tagName);
            }
            var selectedContainer = selector.querySelector('.selected-tags');
            if (selectedContainer.querySelector('.tag-placeholder')) {
              selectedContainer.innerHTML = '';
            ...
            if (selectedTags.indexOf(tagName) < 0) {
              var span = document.createElement('span');
              span.className = 'industry-tag removable';
              span.setAttribute('data-value', tagName);
              span.innerHTML = tagName + '<span class="tag-close">&times;</span>';
              selectedContainer.appendChild(span);
            }
            newInput.value = '';
            render();
  ...
  function showModal(options) {
    ...
    setTimeout(function() {
      try {
        ...
        if (mask.querySelector('.tag-selector')) initTagSelectors(mask);
  ...
  function collectFormData(formEl) {
    ...
    // 收集 tagselect 已选标签
    var tagSelectors = formEl.querySelectorAll('[data-field-type="tagselect"]');
    tagSelectors.forEach(function(selector) {
      var group = selector.closest('.form-group');
      var label = group?.querySelector('.form-label')?.textContent?.replace('*', '').trim();
      if (!label) return;
      var selectedTags = [];
      selector.querySelectorAll('.selected-tags .industry-tag').forEach(function(tag) {
        selectedTags.push(tag.getAttribute('data-value') || tag.textContent.replace('×', '').trim());
      });
      data[label] = selectedTags;
    });
  ...
  function buildFieldHtml(field) {
    ...
    if (field.type === 'select') {
      ...
      // 标签配置：已选区 + 新增输入 + 可选区（与企业新增标签配置保持一致）
      var selectedVals = (field.value && Array.isArray(field.value)) ? field.value : [];
      var selectedTagsHtml = selectedVals.length === 0
        ? '<span class="tag-placeholder">暂无标签，点击下方可选标签添加</span>'
        : selectedVals.map(function(tag) {
            return '<span class="industry-tag removable" data-value="' + tag + '">' + tag + '<span class="tag-close">&times;</span></span>';
          }).join('');
      var availableOptions = (field.options || []).filter(function(opt) { return selectedVals.indexOf(opt) < 0; });
      var availableTagsHtml = availableOptions.length === 0
        ? '<span class="tag-placeholder">所有标签已选中</span>'
        : availableOptions.map(function(opt) {
            return '<span class="industry-tag available" data-value="' + opt + '">+ ' + opt + '</span>';
          }).join('');
      input = '<div class="tag-selector" data-field-type="tagselect">' +
                '<div class="selected-tags">' + selectedTagsHtml + '</div>' +
                '<div class="tag-add-row">' +
                  '<input type="text" class="form-input tag-new-input" placeholder="输入新标签名称，按回车添加" style="flex:1;" />' +
                '</div>' +
                '<div class="available-tags">' +
                  '<div class="available-tags-title">可选标签（点击选择）</div>' +
                  '<div class="available-tags-list">' + availableTagsHtml + '</div>' +
                '</div>' +
              '</div>';
